...
public class Link
{
    ...
    {
        return HttpUtility.UrlPathEncode(String.Format("{0}&business={1}&return={2}&cancel_return={3}&display=1",
                                         BalloonShopConfiguration.PaypalReturnUrl, BalloonShopConfiguration.PaypalEmail,
                                         BalloonShopConfiguration.PaypalReturnUrl, BalloonShopConfiguration.PaypalCancelUrl));
    }
    public static string ToPayPalAddItem(string productUrl, string productName,
                                         decimal productPrice, string productOptions)
    {
        return HttpUtility.UrlPathEncode(String.Format("{0}&business={1}&return={2}&cancel_return={3}&shopping_url={4}&item_name={5}&amount={6:0.00}&currency={7}&onO=Option&osO={8}&add=1",
                                          BalloonShopConfiguration.PaypalUrl, BalloonShopConfiguration.PaypalEmail,
                                          BalloonShopConfiguration.PaypalReturnUrl, BalloonShopConfiguration.PaypalCancelUrl,
                                          productUrl, productName, productPrice,
                                          BalloonShopConfiguration.PaypalCurrency, productOptions));
    }
